...
class BarsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create bar" do
    assert_difference('Bar.count') do
      post bars_url, params: { bar: { address: @bar.address, description: @bar.description, name: @bar.name, tel: @bar.tel } }
    end
  ...
  test "should update bar" do
    patch bar_url(@bar), params: { bar: { address: @bar.address, description: @bar.description, name: @bar.name, tel: @bar.tel } }
